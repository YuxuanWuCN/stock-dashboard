...
5. 三级对照组基准矩阵（沪深300、新能源/绿电ETF、绿电核心6巨头等权买入持有）
"""
...
class GreenBacktestRunner:
    ...
    def load_isolated_raw_data(self) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        if not data_dir.exists():
            ...
            if alt_dir.exists():
                ...
                alt_dir2 = Path(__file__).resolve().parent.parent.parent / "data" / "raw" / "backtest_green_2025q3_2026q3"
                if alt_dir2.exists():
                    data_dir = alt_dir2
    ...
    def run_walk_forward_backtest(self, config: Optional[CalibrationConfig] = None) -> Dict[str, Any]:
        ...
        # 重置校准与回测状态
        self.factor_scores_history = pd.DataFrame(columns=self.GREEN_TICKERS)
        self.returns_history = pd.DataFrame(columns=self.GREEN_TICKERS)
        ...
        for t in range(20, T):
            ...
            if t >= 2:
                prev_date = dates[t-2]
                realized_rets = prices_df[self.GREEN_TICKERS].iloc[t-1] / prices_df[self.GREEN_TICKERS].iloc[t-2] - 1.0
            ...
            if t >= 21 and len(self.calibration_records) >= 1:
                ...
                if prev_record.get("valid_count", 0) > 0 and prev_dt in self.factor_scores_history.index:
                    ...
                    actual_rets_1d = prices_df[self.GREEN_TICKERS].iloc[t] / prices_df[self.GREEN_TICKERS].iloc[t-1] - 1.0
            ...
            open_candidates = [tk for tk in self.GREEN_TICKERS if trend_status[tk] and tk in valid_scores.index]
    ...
    def _calculate_calibration_performance(self, prices_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # 评估有效预测的实际 1 日命中率
        dates = prices_df.index
        rets_1d = prices_df[self.GREEN_TICKERS].pct_change().shift(-1)  # T 日预测对齐 T 日收盘到 T+1 日收盘
    ...
    def generate_and_save_artifacts(
        ...
        # 5. 图 3 · 立新能源 (001258) 绿电特质 Alpha 与 Trend Gate 门控实证
