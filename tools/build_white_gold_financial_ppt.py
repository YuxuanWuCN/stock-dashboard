...
def build_dark_financial_presentation(output_path):
    ...
    cols = [
        ...
        ("通用单体大模型 (GPT-4/DeepSeek)", ["直接让大模型读新闻预测买卖", "秒级（但无法验证）", "❌ 严重数值与财报幻觉 (编造数字)", "❌ 黑盒决策（无法追溯因果）", "❌ 严重时序泄漏（回测虚高）", "❌ 缺乏风控（市场突变实盘暴亏）", "❌ 无法通过金融合规与监管审计"]),
        ("Rainbow-FinGPT (本项目)", ["三层解耦：语义抽取 + 纯数学定价 + 门禁", "约 15 分钟 (全流程自动)", "🌟 100% 坐标锚定，杜绝数值捏造", "🌟 资产定价与特质 Alpha 公式可追溯", "🌟 严格物理时序隔离 (仅使用 <=t 日)", "🌟 Trend Gate C 浪硬门禁清仓防守", "🌟 达观曹植插件 + 自动化低费率"]),
    ]
