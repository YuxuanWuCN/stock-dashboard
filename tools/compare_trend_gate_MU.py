# -*- coding: utf-8 -*-
"""美光科技(MU) 趋势门前后对比 —— 验证趋势过滤对下跌段命中率的改善

对比两个版本：
  v1（无趋势门）：直接用集成投票
  v2（有趋势门）：集成投票 + 趋势门过滤（下跌趋势禁止看多）
"""
import json
import math
import sys
...
for k in ("total", "up", "down", "flat"):
    ...
    if acc is not None:
        print(f"  {label:6s}: 命中 {h}/{t} = {acc*100:.1f}%")
print(f"\n  趋势门抑制次数: {suppressed_count} 次（下跌趋势中禁止的看多信号）")
...
with open(out, "w", encoding="utf-8") as fh:
    json.dump({"v1_baseline": v1_results, "v2_with_gate": v2_results, "suppressed": suppressed_count}, fh, indent=2)
print(f"\n[输出] {out}")
