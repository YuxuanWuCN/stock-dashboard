...
导致面板含 696 个日期（644 真实 + 52 填充）。本脚本直接使用同一张
CSMAR 行情源表中的 ``Filling=0`` 记录作为交易日历，避免引入第二个行情源。
...
DEFAULT_CSMAR_PANEL = (
    PROJECT_ROOT / "data" / "task_split" / "student_c" / "csmar" / "student_c_csmar_factor_panel.parquet"
)
DEFAULT_FILLING_CALENDAR = (
    PROJECT_ROOT
    / "data"
    / "task_split"
    / "student_c"
...
DEFAULT_OUT = (
    PROJECT_ROOT / "data" / "task_split" / "student_c" / "csmar" / "student_c_csmar_factor_panel_filtered.parquet"
)
DEFAULT_OUT_CSV = (
    PROJECT_ROOT / "data" / "task_split" / "student_c" / "csmar" / "student_c_csmar_factor_panel_filtered.csv"
)
DEFAULT_MANIFEST = (
    PROJECT_ROOT / "data" / "task_split" / "student_c" / "csmar" / "student_c_csmar_factor_panel_filtered_manifest.json"
)
...
def parse_args(argv=None):
    ...
    p.add_argument("--filling-calendar", type=Path, default=DEFAULT_FILLING_CALENDAR)
...
def main(argv=None) -> int:
    ...
    filling_calendar = _resolve(args.filling_calendar)
    ...
    if not csmar_path.exists():
        ...
        return 2
    if not filling_calendar.exists():
        print(f"CSMAR Filling 日历不存在: {filling_calendar}", file=sys.stderr)
        return 2
    ...
    trade = pd.read_csv(filling_calendar, dtype={"Filling": str}, encoding="utf-8-sig")
    required_trade_columns = {"TradingDate", "Filling"}
    missing_trade_columns = required_trade_columns - set(trade.columns)
    if missing_trade_columns:
        print(
            "CSMAR Filling 日历缺少交易日标记列: " + ", ".join(sorted(missing_trade_columns)),
            file=sys.stderr,
        )
        return 2

    csmar["trade_date"] = pd.to_datetime(csmar["trade_date"])
    trade["trade_date"] = pd.to_datetime(trade["TradingDate"], errors="coerce")
    filling = trade["Filling"].astype(str).str.strip()

    real_days = set(trade.loc[filling == "0", "trade_date"].dropna().drop_duplicates())
    if not real_days:
        print("CSMAR Filling 日历没有 Filling=0 的真实交易日", file=sys.stderr)
        return 2
    ...
    manifest = {
        "source": "filtered from CSMAR panel using TRD_FwardQuotation.Filling=0",
        "csmar_panel": _display_path(csmar_path),
        "filling_calendar": _display_path(filling_calendar),
        "trade_calendar_source": "CSMAR TRD_FwardQuotation.Filling=0",
