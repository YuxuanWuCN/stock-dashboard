...
def parse_args(argv=None):
    ...
    p.add_argument("--filling-calendar", type=Path, default=None)
...
def main(argv=None) -> int:
    ...
    if filling_calendar is not None and filling_calendar.exists():
        ...
        source_desc = "filtered from CSMAR panel using TRD_FwardQuotation.Filling=0"
    elif public_path is not None and public_path.exists():
        ...
        source_desc = "filtered from CSMAR panel to real trading days"
    else:
        print("交易日历文件不存在: 请指定有效的 --filling-calendar 或 --public-panel", file=sys.stderr)
        return 2
