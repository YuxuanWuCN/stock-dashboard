...
def generate_strict_record(
    ...
    record: dict[str, Any] = {
        ...
        "cohort_key": str(row.get("cohort_key", "student_C")),
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="严格真实来源的同学 C 768D 因子生成")
    parser.add_argument("--sample", type=int, default=None)
    parser.add_argument("--output", type=Path, default=ROOT_DIR / "data/task_split/factors_768d_student_C.csv")
    parser.add_argument(
        "--provenance",
        type=Path,
        default=ROOT_DIR / "data/task_split/factors_768d_student_C_provenance.json",
    )
...
def run(args: argparse.Namespace) -> int:
    ...
    stocks = load_cohort_stocks("student_C")
    if args.sample:
