# -*- coding: utf-8 -*-
"""从已配置的 CSMAR SDK 拉取同学 B 的四张源表并构建标准因子面板。

本脚本只负责认证状态检查、CSMAR 查询和原始源表落盘；字段规范化、股票内填充、
季度 ROE 公告日锚定和最终校验继续复用 ``build_student_b_factor_panel.py``。
...
from scripts import build_student_b_factor_panel as builder  # noqa: E402
...
DEFAULT_OUT_DIR = PROJECT_ROOT / "data" / "task_split" / "student_b" / "csmar"
DEFAULT_RAW_DIR = PROJECT_ROOT / "data" / "task_split" / "student_b" / "sources" / "csmar_raw"
DEFAULT_LOOKBACK_DAYS = 730
OUTPUT_STEM = "student_b_csmar_factor_panel"
...
TABLE_SPECS: dict[str, TableSpec] = {
    "trade": TableSpec(
        ...
        code_column="Symbol",
        columns=("Symbol", "TradingDate", "ClosePrice", "Volume", "TurnoverRate1", "MarketValue"),
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="拉取同学 B 的 CSMAR 四张源表并构建因子面板")
...
def fetch_and_build(args: argparse.Namespace, service_factory: Callable[[], Any] | None = None) -> int:
    ...
    try:
        ...
        print(f"同学 B CSMAR 数据任务失败：{exc}", file=sys.stderr)
        return 2
