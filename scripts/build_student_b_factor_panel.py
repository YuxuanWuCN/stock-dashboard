# -*- coding: utf-8 -*-
"""scripts/build_student_b_factor_panel.py —— 构建同学 B（能源与材料组）100 只标的标准因子面板

任务池为绿电与清洁公用 + 黄金有色与周期资源共 100 只标的，窗口 2024-01-02 ~ 2026-08-28。
...
需要从 CSMAR 导出的表（学校账号在机房 Python 环境导出 CSV/Parquet，本脚本零配置热加载）：
  ...
  日频行情   STK_TRADE_DAYADJ   SecuCode, TrdTrdDt, TrdClsPrc, TrdVol_Total_A, TrdTNRate_Total_A
  日频估值   STK_PE_ADJ         SecuCode, TrdTrdDt, PE_TTM, PBR
  日频市值   STK_CAPITALS       SecuCode, CapDate, MV_Total_A
  季度回报   STK_FIN_Anlysis    SecuCode, ReportDate, ROE_TTM, AnnounceModifyDate
...
DEFAULT_TASK_FILE = PROJECT_ROOT / "data" / "task_split" / "student_B_energy_materials_100.csv"
DEFAULT_START = "2024-01-02"
DEFAULT_END = "2026-08-28"
DEFAULT_OUT_DIR = PROJECT_ROOT / "data" / "task_split" / "student_b"
DEFAULT_STEM = "student_b_factor_panel"
REQUIRED_COLUMNS = list(FACTOR_COLUMNS)

#: 组别标识，写入 manifest 便于三方结果比对与追溯。
STUDENT_GROUP = "同学 B（能源与材料组）"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="构建同学 B（能源与材料组）100 只标的标准因子面板",
...
def _build_manifest(
    ...
    return {
        ...
        "task_file": str(args.task_file.relative_to(PROJECT_ROOT) if args.task_file.is_relative_to(PROJECT_ROOT) else args.task_file),
        "codes": codes,
        "n_stocks": len(codes),
        ...
        "csmar_tables_to_export": [
            "STK_TRADE_DAYADJ (SecuCode, TrdTrdDt, TrdClsPrc, TrdVol_Total_A, TrdTNRate_Total_A)",
            "STK_PE_ADJ (SecuCode, TrdTrdDt, PE_TTM, PBR)",
            "STK_CAPITALS (SecuCode, CapDate, MV_Total_A)",
            "STK_FIN_Anlysis (SecuCode, ReportDate, ROE_TTM, AnnounceModifyDate)",
        ],
    }
    ...
    try:
        ...
        if not sources:
            print(
                ...
                "公开行情过渡源可用 scripts/fetch_student_b_market_data.py 拉取。\n"
            )
            return 2
        ...
        if manifest["coverage_status"] != "complete":
            ...
            print("\n补齐方式：见 python scripts/build_student_b_factor_panel.py --help 中的 CSMAR 表清单；"
