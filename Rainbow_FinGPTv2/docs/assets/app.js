...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderPaper() {
        var allPortfolios = state.paperSeries || [];
        if (!allPortfolios.length) {
            el.paperMeta.textContent = '暂无模拟盘数据，每日收盘后自动记录。';
            return;
        }

        // 优先展示 6 大基础模拟盘 + 全池等权基准，衍生变体分组展示
        var basePortfolios = allPortfolios.filter(function (s) {
            return !s.isVariant && s.points.length >= 2;
        });
        var portfolios = basePortfolios.length ? basePortfolios : allPortfolios;

        var starts = portfolios.map(function (s) {
            return s.points.length ? s.points[0].date : null;
        }).filter(Boolean);
        el.paperMeta.textContent = starts.length
            ? ('对照起点：' + starts[0] + '（共 ' + portfolios[0].points.length + ' 交易日长跑实测），每日 18:00 自动结算。')
            : '';

        el.paperCards.innerHTML = allPortfolios.map(function (s) {
            ...
            var tagBadge = s.isBenchmark ? '<span style="background:#64748b;color:#fff;font-size:10px;padding:2px 6px;border-radius:4px;margin-left:6px;">基准</span>' :
                          (s.isVariant ? '<span style="background:#f59e0b;color:#fff;font-size:10px;padding:2px 6px;border-radius:4px;margin-left:6px;">变体</span>' : '<span style="background:#0284c7;color:#fff;font-size:10px;padding:2px 6px;border-radius:4px;margin-left:6px;">核心</span>');
            return '<div class="paper-card" style="border-top-color:' + s.color + '">' +
                '<div class="paper-card-name">' + (s.isBenchmark ? '📊 ' : '💼 ') + s.name + tagBadge + '</div>' +
                '<div class="paper-card-row">当日收益 <span class="paper-card-value" style="color:' + (last && last.daily >= 0 ? '#16a34a' : '#dc2626') + '">' + _paperPct(last ? last.daily : null) + '</span></div>' +
                '<div class="paper-card-row">累计净值 <span class="paper-card-value" style="color:' + (total >= 0 ? '#16a34a' : '#dc2626') + '">' + _paperPct(total) + ' (NAV ' + (last ? last.nav.toFixed(2) : '--') + ')</span></div>' +
                '<div class="paper-card-row" style="font-size:11px;color:#94a3b8;">' + s.points.length + ' 交易日' + (first ? ' (' + first.date + ' ~ ' + last.date + ')' : '') + '</div>' +
                '</div>';
        }).join('');

        renderPaperCurve(allPortfolios);

        var thead = document.querySelector('#paper-compare-wrap thead tr');
        if (thead) {
            thead.innerHTML = '<th>日期</th>' + allPortfolios.map(function (s) {
                return '<th>' + s.name + '</th>';
            }).join('');
        }
        var dates = {};
        allPortfolios.forEach(function (s) {
        ...
        if (dateList.length) {
            ...
            el.paperCompareTbody.innerHTML = dateList.slice().reverse().map(function (d) {
                return '<tr><td>' + d + '</td>' + allPortfolios.map(function (s) {
    ...
    function renderPaperCurve(portfolios) {
        var W = 720, H = 280, padL = 50, padR = 20, padT = 20, padB = 32;
        var allDates = {};
        ...
        // 针对不同起点日期的组合，进行起点前平滑（未开始前保持 NAV 100）
        function align(s) {
            var map = {};
            s.points.forEach(function (p) { map[p.date] = p.nav; });
            var firstDate = s.points.length ? s.points[0].date : null;
            var last = 100;
            var started = false;
            return dates.map(function (d) {
                if (d === firstDate) started = true;
                if (!started) return 100.0;
                last = map[d] != null ? map[d] : last;
                return last;
            });
        }
        var lines = portfolios.map(function (s) {
            return { key: s.key, name: s.name, color: s.color, vals: align(s), isBenchmark: s.isBenchmark, isVariant: s.isVariant };
        });
        var allVals = [100];
        ...
        for (var g = 0; g <= 4; g++) {
            ...
            gridY += '<line x1="' + padL + '" y1="' + gy + '" x2="' + (W - padR) + '" y2="' + gy + '" stroke="#e2e8f0" stroke-width="1" stroke-dasharray="3,3"/>' +
                '<text x="' + (padL - 8) + '" y="' + (gy + 4) + '" text-anchor="end" font-size="11" font-family="monospace" fill="#64748b">' + gv.toFixed(1) + '</text>';
        }
        var legend = lines.map(function (l) {
            var strokeStyle = l.isBenchmark ? 'dashed' : 'solid';
            return '<span class="paper-legend-item"><i style="background:' + l.color + ';border-style:' + strokeStyle + '"></i>' + l.name + '</span>';
        }).join('');
        var xEvery = Math.max(1, Math.floor(dates.length / 6));
        var xLabels = dates.map(function (d, i) {
            if (i % xEvery !== 0 && i !== dates.length - 1) return '';
            return '<text x="' + x(i).toFixed(1) + '" y="' + (H - 8) + '" text-anchor="middle" font-size="10" fill="#64748b">' + d.slice(5) + '</text>';
        }).join('');
        var svg = '<svg viewBox="0 0 ' + W + ' ' + H + '" role="img" aria-label="累计净值曲线" style="width:100%;height:auto;display:block;">' +
            gridY + xLabels +
            lines.map(function (l) {
                var w = l.isBenchmark ? "2.5" : (l.isVariant ? "1.2" : "2.0");
                var dash = l.isBenchmark ? 'stroke-dasharray="4,4"' : '';
                return '<path d="' + path(l.vals) + '" fill="none" stroke="' + l.color + '" stroke-width="' + w + '" ' + dash + ' stroke-linejoin="round" stroke-linecap="round"/>';
            }).join('') +
