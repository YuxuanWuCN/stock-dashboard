...
核心特性：
1. 严格 5 级规范字阶体系 (T1 主标题 ~ T5 表格数据)，彻底消除字体字号混乱
2. 统一字体映射：Microsoft YaHei 家族无衬线排版，中英文与数字完美对齐
3. 专业数据表格渲染：表头主题底色、交替斑马纹 (#F8FAFC / #FFFFFF)、0.15mm 极细高精边框、舒适内边距
4. 商业级 Accent Callout 叙事框：淡底色 + 左侧 2.5mm 主题色重色指示条
5. 现代化 KPI 磁贴网格：高对比度指标卡片
"""
...
class BasePublicationPDF(FPDF):
    ...
    def header(self):
        ...
        self.set_font(self.font_regular, "", self.FS_FOOTER)
    ...
    def draw_section_header(self, text: str, y_offset: Optional[float] = None):
        ...
        if y_offset is not None:
            self.set_y(y_offset)
        self.set_font(self.font_bold, "", self.FS_SECTION)
    ...
    def draw_kpi_cards(self, kpis: List[Tuple[str, str, Tuple[int, int, int]]], y_pos: float):
        ...
        for i, (lbl, val, color) in enumerate(kpis):
            ...
            self.set_font(self.font_regular, "", self.FS_CARD_LBL)
            self.set_text_color(100, 116, 139)
            self.cell(card_w, 3.0, lbl, align="C")

            # 大数值
            self.set_xy(x, y_pos + 4.5)
            self.set_font(self.font_bold, "", self.FS_CARD_VAL)
            self.set_text_color(*color)
            self.cell(card_w, 5.0, val, align="C")

    def draw_accent_box(self, x: float, y: float, w: float, h: float, text: str, line_h: float = 3.2):
        """绘制带有左侧 Accent Bar 的商业级说明框。"""
        # 主背景
        self.set_fill_color(248, 250, 252)
        self.set_draw_color(226, 232, 240)
        self.set_line_width(0.2)
        ...
        # 文字
        self.set_xy(x + 4.0, y + 1.2)
        self.set_font(self.font_regular, "", self.FS_BODY)
        self.set_text_color(30, 41, 59)
        self.multi_cell(w - 6.0, line_h, text)
    ...
    def draw_styled_table(
        ...
        self.set_line_width(0.2)
        self.set_font(self.font_bold, "", self.FS_TABLE_HDR)
        ...
        for idx, row in enumerate(rows):
            ...
            if is_highlight:
                self.set_font(self.font_bold, "", self.FS_TABLE_CELL)
                self.set_text_color(*self.theme_color)
            else:
                self.set_font(self.font_regular, "", self.FS_TABLE_CELL)
                self.set_text_color(30, 41, 59)
